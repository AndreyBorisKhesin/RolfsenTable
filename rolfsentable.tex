...
Next, we just need to construct the non-alternating knots by flipping the signs
of the crossings of each of the alternating knots in every possible way.
There would be $2^{n-1}$ ways of flipping these crossings for each alternating
knot since the first crossing must remain positive.
...
non-alternating knot diagrams that we had left to consider.
...
Then, for each knot diagram $K$, we connect its vertex to all of the vertices
that represent knot diagrams that $K$ can be transformed into with a single move
(see \figMoves).
If $K$ can be transformed into a knot diagram whose vertex is not in the graph,
that vertex is added to the graph.
This is important for removing the remaining reducible knot diagrams from the
graph.
